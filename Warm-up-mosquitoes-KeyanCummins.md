...
<img src="Warm-up-mosquitoes-KeyanCummins_files/figure-gfm/viremia-1.png" style="display: block; margin: auto auto auto 0;" />
...
mosquito’s blood meal from multiple locations and then used Polymerase
Chain Reaction (PCR) to amplify a target gene. After PCR, we sequenced
the amplified DNA and referenced it to a database to identify the host
species. Using the data from the identified hosts and the WNV positivity
rates in the local mosquito pools helps us to understand more clearly
the transmission of WNV in these areas.
...
classified as either WNV-negative or WNV-positive.

## 1st Analysis (Barplots)

We counted the total number of blood meals taken from each host species.
We added the counts separately for WNV-negative and WNV-positive
locations. We visualized this comparison using horizontal bar plots to
show host feeding patterns directly. The bar plots helped us see if any
specific host was targeted more by mosquitoes in WNV-positive areas.
...
![](Warm-up-mosquitoes-KeyanCummins_files/figure-gfm/first-analysis-1.png)<!-- -->
...
To formally test the relationship shown in the barplots, we performed a
Generalized Linear Model (GLM) with the number of House Finch blood
meals as the independent variable. We ran two separate models. The first
model used the binary presence of WNV (loc_positives) as the response
variable with a binomial family. The second model used the WNV
positivity rate (loc_rate) as the response variable with a Gaussian
family. This statistical test gave us a formal p-value to assess the
...
The second analysis, a generalized linear model, confirmed the pattern
seen in the barplots with statistics. The model that tested whether WNV
was present showed a significant positive result for House Finch
(p=0.0287). This indicates that locations with more House Finch blood
meals were much more likely to have WNV-positive mosquito pools. Also,
the model that examined the WNV positivity rate found a highly
significant positive relationship (p=4.54×10^−5). This means that more
House Finch blood meals lead to a statistically significant rise in the
WNV infection rate at a location. Both models support the idea that
House Finches play a key role in spreading WNV.
...
The conclusion we found for the research question is that the House
Finch is likely the most important host for West Nile virus in Salt Lake
City, Utah. The evidence strongly supports the hypothesis, showing that
the number of House Finch blood meals is a significant positive
predictor of WNV presence and positivity rate at a mosquito collection
site. This finding aligns with biological data indicating that House
Finches maintain a high viremia \[1\], confirming their role as a key
host needed for the virus to spread effectively through the mosquito
population in the Salt Lake City area. Public health measures should use
this finding to prioritize mosquito control efforts in areas with large
...
2.  Salt Lake County Health Department. (n.d.). West Nile virus detected
    in multiple mosquito samples in Salt Lake County. Retrieved from
